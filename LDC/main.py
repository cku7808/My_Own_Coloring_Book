...
from dataset import DATASET_NAMES, TrainDataset, TestDataset, dataset_info
# from loss import *
from loss2 import *
...
def validate_one_epoch(epoch, dataloader, model, device, output_dir, arg=None):
    ...
    with torch.no_grad():
        for _, sample_batched in enumerate(dataloader):
            ...
            print("file_names",file_names,"image_shape",image_shape)
            preds = model(images)
            print(images.shape)
            # try:
            #     preds = model(images)
...
def test(checkpoint_path, dataloader, model, device, output_dir, args):
    ...
    with torch.no_grad():
        ...
        for batch_id, sample_batched in enumerate(dataloader):
            ...
            if not args.test_data == "Paint_test":
                labels = sample_batched['labels'].to(device)
            file_names = sample_batched['file_names']
            image_shape = sample_batched['image_shape']
...
def parse_args():
    ...
    TRAIN_DATA = DATASET_NAMES[25] # Transfer=14, paint_train=16, np_train=19, gan_train=22, land_train=25
    ...
    parser.add_argument('--checkpoint_data',
                        type=str,
                        default='24/24_model.pth',# 37 for biped 60 MDBD
                        # default=None,
...
def main(args):
    ...
    if not args.is_testing:
        if args.resume:
            ...
            checkpoint_path2= os.path.join(args.output_dir, args.train_data,args.checkpoint_data)
            print("load checkpoint path",checkpoint_path2)
            ini_epoch=25
            model.load_state_dict(torch.load(checkpoint_path2,
                                         map_location=device))
        dataset_train = TrainDataset(args.input_dir,
